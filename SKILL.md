...
本 skill 依赖以下 sub-agent prompt，以源文件形式捆绑在 `agents/` 目录下：
...
- `agents/proxy-agent.md` — 通用外部模型派发 subagent，2.4b validator 走 `BACKEND=deepseek-chat` 时调用

以及以下顶层 skill，捆绑在 `skills/` 目录下：

- `skills/abagent/` — 外部模型派发的知识层与调用范例（搭配 `proxy-agent` 使用）

Claude Code 从 `~/.claude/agents/` 和 `~/.claude/skills/` 发现 sub-agent 与 skill 定义，所以首次 install 时必须把这些目录同步过去：

```bash
bash scripts/install-agents.sh            # agents/*.md → ~/.claude/agents/（symlink，改 repo 立即生效）
bash scripts/install-skills.sh            # skills/*/  → ~/.claude/skills/（symlink）
bash scripts/install-agents.sh --copy     # copy 模式（跨文件系统 / 不想跟 submodule 联动时用，install-skills.sh 同）
```

已存在且内容不同的 `~/.claude/agents/*.md` 或 `~/.claude/skills/*/` 会被**跳过**，不覆盖。

`proxy-agent` 与 `abagent` 是为本 skill 服务但可被其他调用方复用的通用基础设施；真本归属在本仓内，外部调用方通过本 submodule 同步。修改后跑一次 install 脚本即可生效（symlink 模式自动跟随）。

不要把这些 prompt 或 skill 重新内联回 `SKILL.md`。修改 session 卡片、facet schema 或 proxy-agent 契约时，改 `agents/` / `skills/` 下对应文件和脚本测试。

## Sub-agent Model Policy

- 轻量、机械、低推理深度任务使用 Haiku：隐私审查、全文隐私复审、临时脚本执行子 agent。
- 候选 validator（2.4b）走 `proxy-agent` + `BACKEND=deepseek-chat`，不消耗 Anthropic token；详见 `reference/workflows/02-write-review/04-candidates.md`。
